...
c_tensor_ptr_p = ctypes.POINTER(c_tensor_ptr)


def _bind_optional(name: str, restype, argtypes):
    try:
...
_ga_tensor_data = _bind_optional("ga_tensor_data", c_void_p, [c_tensor_ptr])
...
_ga_neg = _bind_optional("ga_neg", c_tensor_ptr, [c_tensor_ptr])
_ga_exp = _bind_optional("ga_exp", c_tensor_ptr, [c_tensor_ptr])
_ga_log = _bind_optional("ga_log", c_tensor_ptr, [c_tensor_ptr])
_ga_sqrt = _bind_optional("ga_sqrt", c_tensor_ptr, [c_tensor_ptr])
_ga_abs = _bind_optional("ga_abs", c_tensor_ptr, [c_tensor_ptr])
_ga_cat = _bind_optional("ga_cat", c_tensor_ptr, [c_tensor_ptr_p, c_int, c_int])
_ga_stack = _bind_optional("ga_stack", c_tensor_ptr, [c_tensor_ptr_p, c_int, c_int])
...
class Tensor:
    ...
    def numpy(self, copy: bool = True) -> np.ndarray:
        """Convert to numpy array. Set copy=False to request a zero-copy view when possible."""
        if not copy and _ga_tensor_data is not None:
            try:
                if _lib.ga_tensor_is_contiguous(self._c_ptr):
                    dtype = np.dtype(self.dtype)
                    ctype = _CTYPE_MAP.get(dtype)
                    if ctype is not None:
                        ptr = _ga_tensor_data(self._c_ptr)
                        if ptr:
    ...
    def data(self, value: Union[np.ndarray, List, float, int]) -> None:
        arr = np.ascontiguousarray(np.asarray(value, dtype=self.dtype))
    ...
    def transpose(self) -> "Tensor":
        ...
        if new_ptr:
            ...
            out = Tensor(_c_ptr=new_ptr, _shape=new_shape, _dtype=self._dtype, _requires_grad=False)
        else:
            # Fallback to numpy if C call fails
            arr = self.numpy().T
            out = Tensor(arr.astype(self.dtype), dtype=self.dtype, _requires_grad=False)
        requires_grad = self.requires_grad and _is_grad_enabled()
    ...
    def __neg__(self) -> "Tensor":
        ptr = None
        if _ga_neg is not None:
            try:
                ptr = _ga_neg(self._c_ptr)
            except Exception:
                ptr = None
        if ptr:
    ...
    def __add__(self, other: Union["Tensor", float, int]) -> "Tensor":
        ...
        if isinstance(other, Tensor):
            ...
            if result_ptr:
                ...
                out = Tensor((self.numpy() + other.numpy()).astype(self.dtype), dtype=self.dtype)
        ...
        if result_ptr:
            ...
            out = Tensor((self.numpy() + float(other)).astype(self.dtype), dtype=self.dtype)
    ...
    def __truediv__(self, other: Union["Tensor", float, int]) -> "Tensor":
        ...
        if result_ptr:
            ...
            out = Tensor((self.numpy() * scale).astype(self.dtype), dtype=self.dtype)
        if _is_grad_enabled() and self.requires_grad:
            out._requires_grad = True
            out._prev = (self,)
    ...
    def softmax(self, dim: int = -1) -> "Tensor":
        ...
        dim_resolved = dim if dim >= 0 else self.ndim + dim
        result_ptr = None
        if dim_resolved == self.ndim - 1:
            try:
                result_ptr = _lib.ga_softmax(self._c_ptr, int(dim_resolved))
            except Exception:
                result_ptr = None
        arr_out = None
        if result_ptr:
    ...
    def exp(self) -> "Tensor":
        result_ptr = None
        if _ga_exp is not None:
            try:
                result_ptr = _ga_exp(self._c_ptr)
            except Exception:
                result_ptr = None
        if result_ptr:
            out = Tensor(_c_ptr=result_ptr, _dtype=self.dtype)
        else:
            out = Tensor(np.exp(self.numpy()).astype(self.dtype), dtype=self.dtype)
        requires_grad = _is_grad_enabled() and self.requires_grad
        out._requires_grad = requires_grad
        if requires_grad:
            out._prev = (self,)
    ...
    def log(self) -> "Tensor":
        result_ptr = None
        if _ga_log is not None:
            try:
                result_ptr = _ga_log(self._c_ptr)
            except Exception:
                result_ptr = None
        if result_ptr:
            out = Tensor(_c_ptr=result_ptr, _dtype=self.dtype)
    ...
    def sqrt(self) -> "Tensor":
        result_ptr = None
        if _ga_sqrt is not None:
            try:
                result_ptr = _ga_sqrt(self._c_ptr)
            except Exception:
                result_ptr = None
        if result_ptr:
            out = Tensor(_c_ptr=result_ptr, _dtype=self.dtype)
        else:
            out = Tensor(np.sqrt(self.numpy()).astype(self.dtype), dtype=self.dtype)
        requires_grad = _is_grad_enabled() and self.requires_grad
        out._requires_grad = requires_grad
        if requires_grad:
            out._prev = (self,)
    ...
    def abs(self) -> "Tensor":
        result_ptr = None
        if _ga_abs is not None:
            try:
                result_ptr = _ga_abs(self._c_ptr)
            except Exception:
                result_ptr = None
        if result_ptr:
            out = Tensor(_c_ptr=result_ptr, _dtype=self.dtype)
...
# ----------------------------------------------------------------------
def _validate_tensor_list(tensors: Iterable["Tensor"]) -> List["Tensor"]:
    items = list(tensors)
    if not items:
        raise ValueError("Expected at least one Tensor")
    if not all(isinstance(t, Tensor) for t in items):
...
def stack(tensors: Iterable["Tensor"], dim: int = 0) -> Tensor:
    """Stack tensors along a new dimension (C-backed when available)."""
    items = _validate_tensor_list(tensors)
    base_shape = items[0].shape
    for t in items[1:]:
        if t.shape != base_shape:
            raise ValueError("All tensors must have the same shape for stack")
    dim_resolved = dim if dim >= 0 else len(base_shape) + 1 + dim
    if dim_resolved < 0 or dim_resolved > len(base_shape):
        raise ValueError("Stack dim out of range")
    if _ga_stack is not None:
        ptrs = (c_tensor_ptr * len(items))(*(t._c_ptr for t in items))
        try:
            result_ptr = _ga_stack(ptrs, len(items), int(dim_resolved))
        except Exception:
            result_ptr = None
        if result_ptr:
            out_shape = list(base_shape)
            out_shape.insert(dim_resolved, len(items))
            return Tensor(_c_ptr=result_ptr, _shape=tuple(out_shape), _dtype=items[0].dtype)
    arrs = [t.numpy() for t in items]
    return Tensor(np.stack(arrs, axis=dim_resolved).astype(items[0].dtype), dtype=items[0].dtype)


def cat(tensors: Iterable["Tensor"], dim: int = 0) -> Tensor:
    """Concatenate tensors along an existing dimension (C-backed when available)."""
    items = _validate_tensor_list(tensors)
    base_shape = items[0].shape
    dim_resolved = dim if dim >= 0 else len(base_shape) + dim
    if dim_resolved < 0 or dim_resolved >= len(base_shape):
        raise ValueError("Concat dim out of range")
    for t in items[1:]:
        ...
        for axis, (a, b) in enumerate(zip(base_shape, t.shape)):
            if axis == dim_resolved:
                continue
            if a != b:
                raise ValueError("All tensors must match shape except along concat dim")
    if _ga_cat is not None:
        ptrs = (c_tensor_ptr * len(items))(*(t._c_ptr for t in items))
        try:
            result_ptr = _ga_cat(ptrs, len(items), int(dim_resolved))
        except Exception:
            result_ptr = None
        if result_ptr:
            out_shape = list(base_shape)
            out_shape[dim_resolved] = sum(t.shape[dim_resolved] for t in items)
            return Tensor(_c_ptr=result_ptr, _shape=tuple(out_shape), _dtype=items[0].dtype)
    arrs = [t.numpy() for t in items]
    return Tensor(np.concatenate(arrs, axis=dim_resolved).astype(items[0].dtype), dtype=items[0].dtype)
...
def cross_entropy(logits: Tensor, target: Tensor, reduction: str = "mean") -> Tensor:
    if getattr(_lib, "ga_cross_entropy_loss", None) and logits.dtype == np.float32:
        if target.dtype != np.int64:
            target = Tensor(target.numpy().astype(np.int64), dtype=np.int64)
        red_code = _to_reduction_code(reduction)
        ptr = _lib.ga_cross_entropy_loss(logits._c_ptr, target._c_ptr, red_code)
        if not ptr:
            raise RuntimeError("Cross entropy loss failed")
        out = Tensor(_c_ptr=ptr, _dtype=np.float32, _requires_grad=_is_grad_enabled() and logits.requires_grad)
        return out
...
def from_numpy(arr: np.ndarray) -> Tensor:
    """Create a zero-copy Tensor view from a numpy array."""
    ...
    dtype_code = _DTYPE_MAP.get(arr.dtype, 0)
    try:
        ptr = _lib.ga_tensor_from_data(ndim, shape_arr, dtype_code, arr.ctypes.data)
    except Exception:
        ptr = None
    if not ptr:
        raise MemoryError("Failed to create tensor view")
    t = Tensor(_c_ptr=ptr, _shape=arr.shape, _dtype=arr.dtype, _requires_grad=False)
    t._ctx_ref = arr
    return t
