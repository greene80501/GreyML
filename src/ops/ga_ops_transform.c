...
GATensor* ga_cat(GATensor** tensors, int num_tensors, int dim) {
    ...
    for (int i = 0; i < num_tensors; i++) {
        ...
        int64_t copy_elems = t->size;
        memcpy(out_data + offset, t->data, copy_elems * sizeof(float));
        offset += copy_elems;
        ga_tensor_release(t);
    }
    return out;
...
void ga_split(GATensor* tensor, int chunks, int dim, GATensor** outputs) {
    ...
    for (int c = 0; c < chunks; c++) {
        ...
        float* dst = (float*)out->data;
        float* src = (float*)tensor->data + offset_elems;
        memcpy(dst, src, out->size * sizeof(float));
        outputs[c] = out;
