...
static void select_feature_subset(int* idx, int total, int choose) {
    ...
    for (int i = 0; i < choose; i++) {
        int j = i + (int)(ga_random_uint32() % (uint32_t)(total - i));
        int tmp = idx[i];
        idx[i] = idx[j];
        idx[j] = tmp;
    }
    ...
    if (!is_regression) {
        ...
        for (int fi = 0; fi < choose; fi++) {
            ...
            for (int64_t i = 0; i < n_samples; i++) {
                ...
                pairs[i].value = Xd[row * n_features + f];
                pairs[i].target = (float)yd[row];
            }
        ...
        bool y_is_f32 = (y->dtype == GA_FLOAT32);
        float* yd_f = (float*)y->data;
        int64_t* yd_i = (int64_t*)y->data;
        for (int fi = 0; fi < choose; fi++) {
            int f = feat_idx[fi];
            ...
            for (int64_t i = 0; i < n_samples; i++) {
                int64_t row = indices[i];
                float yv = y_is_f32 ? yd_f[row] : (float)yd_i[row];
                total_sum += yv;
                total_sumsq += yv * yv;
                pairs[i].value = Xd[row * n_features + f];
                pairs[i].target = yv;
...
GATreeSplit ga_tree_find_best_split(GATensor* X, GATensor* y, int n_classes, GATreeCriterion criterion, int max_features, int min_samples_leaf, bool is_regression) {
    ...
    int64_t* indices = (int64_t*)malloc(sizeof(int64_t) * (size_t)n_samples);
    for (int64_t i = 0; i < n_samples; i++) indices[i] = i;
    GATreeSplit best = ga_tree_find_best_split_indexed(X, y, indices, n_samples, n_classes, criterion, max_features, min_samples_leaf, is_regression);
    free(indices);
    return best;
}
